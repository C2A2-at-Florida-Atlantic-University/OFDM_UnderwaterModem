...
sync_samples_early = 60;         % Number of samples to synchronize early
...
pilot_index=[zp_carriers/2:rep:nfft/2-1,nfft/2+2:rep:nfft- ...
    zp_carriers/2-cp_carriers];
if zp_index(end/2+2)-cp_carriers/2-1 ~= pilot_index(end)
    pilot_index = [pilot_index zp_index(end/2+2)-cp_carriers-1];
end
pilot_carriers = length(pilot_index);
cp_index = pilot_index(end)+1:(nfft-zp_carriers/2+1);
pilot_cp_max = floor(cp_carriers/rep);
pilot_index_cp = [pilot_index (pilot_index(1:pilot_cp_max)+ ...
    pilot_index(end))-zp_carriers/2+1];
data_index = 1:nfft;
data_index([zp_index,pilot_index,cp_index]) = [];
data_carriers = length(data_index);
data_cp_max = cp_carriers-pilot_cp_max;
...
pilot_data_cp = [pilot_data;pilot_data(1:pilot_cp_max,:)];
...
tx_data_cp = [tx_data;tx_data(1:data_cp_max,:)];
...
carriers(pilot_index,:) = reference_pilot;         % Allocate Pilots
carriers(data_index,:) = qam_mod_data;             % Allocate Data
carriers(zp_index,:) = zp_carrier;                 % Allocate Zeros
freq_cp_data = zeros(pilot_carriers+data_carriers,ofdm_symbols);
freq_cp_data(pilot_index,:) = reference_pilot;
freq_cp_data(data_index,:) = qam_mod_data;
freq_cp_data(cp_carriers+1:end,:) = [];
carriers(cp_index,:) = freq_cp_data;               % Allocate CP carriers
reference_pilot_cp = [reference_pilot;freq_cp_data(1:rep:cp_carriers,:)];
carriers = carriers * tx_scale;

zp_plot=zeros(1,nfft);pilot_plot=zeros(1,nfft);data_plot=zeros(1,nfft);
cp_plot=zeros(1,nfft);zp_plot(zp_index)=1;pilot_plot(pilot_index)=1;
data_plot(data_index)=1;cp_plot(cp_index)=1;
figure(),bar(1:nfft,zp_plot),hold on,bar(1:nfft, ...
    pilot_plot,'k'),bar(1:nfft,data_plot)
bar(1:nfft,cp_plot),legend('ZP','Pilot','Data','CP')
title('Resource Allocation'),xlabel('Sub-Carriers')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % DDC
    ddc_dopler_real2 = decimate(real(ddc_signal_dopler2), ...
        Interp_val,4096,'Fir');
    ddc_dopler_imag2 = decimate(imag(ddc_signal_dopler2), ...
        Interp_val,4096,'Fir');
...
% Frequency domain CP
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
freq_cp = Z_EQ_ZF(cp_index,:);
fft_signal_cp = Z_EQ_ZF; cp_freq_domain = zeros(1,ofdm_symbols);
for i = 1:ofdm_symbols
    cp_begin =fft_signal_cp(pilot_index(1)+dopler_cp_offset: ...
        pilot_index(1)+cp_carriers-1-dopler_cp_offset,i);
    cp_end = freq_cp(dopler_cp_offset+1:end-dopler_cp_offset,i);
    tmp = cp_begin ./ conj(cp_end);
    tmp2 = mean(tmp);
    Z_EQ_ZF = Z_EQ_ZF .* exp(1i*pi*2*angle(tmp2));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Take out non data subcarriers
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tmp_val = Z_EQ_ZF(cp_index,:);
Z_EQ_ZF_tmp = Z_EQ_ZF;
Z_EQ_ZF_tmp([pilot_index,zp_index,cp_index],:) = [];
Z_EQ_ZF([pilot_index,zp_index],:) = [];
fft_signal_eq([pilot_index,zp_index,cp_index],:) = [];
fft_dopler_eq([pilot_index,zp_index,cp_index],:) = [];
Z_EQ_ZF_dopler([pilot_index,zp_index,cp_index],:) = [];
fft_dopler_eq_no_cfo([pilot_index,zp_index,cp_index],:) = [];
Z_EQ_ZF_dopler_no_cfo([pilot_index,zp_index,cp_index],:) = [];
fft_dopler_eq2([pilot_index,zp_index,cp_index],:) = [];
Z_EQ_ZF_dopler2([pilot_index,zp_index,cp_index],:) = [];
figure(),subplot(2,1,1),scatter(real(Z_EQ_ZF_tmp),imag(Z_EQ_ZF_tmp),'.')
title('Z EQ ZF'),subplot(2,1,2),scatter(real(tmp_val),imag(tmp_val),'.')
title('CP Subcarriers')
...
ser_data = symerr(tx_data_cp, demod_data)/numel(tx_data_cp);
