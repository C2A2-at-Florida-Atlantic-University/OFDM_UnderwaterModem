...
cp_len = 256;               % Cyclic Prefix length in samples
...
for i=1:ofdm_symbols
    for k=1:nfft
        fft_doppler_eq2(k,i) = fft_doppler_eq2(k,i) * exp(-1j*2*pi*cfo*(k-1)/nfft);
    end
end
...
figure(),subplot(2,1,1),plot(real(TA_comp(:,1))),title('In-Phase')
xlabel('Sub-Carrier Index'),subplot(2,1,2),plot(imag(TA_comp(:,1)))
title('Quadrature'),xlabel('Sub-Carrier Index'),sgtitle('TA Compensation')
...
% e_cfo = zeros(,ofdm_symbols-1);
% for i=1:ofdm_symbols-1
%     e_cfo(i) = H(:,i+1).*conj(H(:,i));
% end
