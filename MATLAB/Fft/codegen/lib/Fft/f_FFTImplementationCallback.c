 ...
 * File: f_FFTImplementationCallback.c
 *
...
static void f_b_FFTImplementationCallback_r(const emxArray_creal_T *x,
                                            int n1_unsigned,
                                            ...
                                            emxArray_creal_T *y)
{
  ...
  f_emxEnsureCapacity_creal_T(y, iDelta2);
  y_data = y->data;
                                     ...
                                     emxArray_creal_T *y)
{
  ...
  x_data = x->data;
  nInt2m1 = (nfft + nfft) - 1;
  f_emxInit_creal_T(&wwc, 1);
  i = wwc->size[0];
  wwc->size[0] = nInt2m1;
  f_emxEnsureCapacity_creal_T(wwc, i);
  wwc_data = wwc->data;
  rt = 0;
  ...
  f_emxInit_creal_T(&fv, 1);
  i = fv->size[0];
  fv->size[0] = nfft;
  f_emxEnsureCapacity_creal_T(fv, i);
  fv_data = fv->data;
  if (nfft > x->size[0]) {
    i = fv->size[0];
    fv->size[0] = nfft;
  ...
  i = y->size[0];
  y->size[0] = fv->size[0];
  f_emxEnsureCapacity_creal_T(y, i);
  y_data = y->data;
  ...
  f_FFTImplementationCallback_r2b(y, n2blue, costab, sintab, fv);
  fv_data = fv->data;
  f_emxInit_creal_T(&b_fv, 1);
  f_FFTImplementationCallback_r2b(wwc, n2blue, costab, sintab, b_fv);
  i = b_fv->size[0];
  b_fv->size[0] = fv->size[0];
  f_emxEnsureCapacity_creal_T(b_fv, i);
  b_fv_data = b_fv->data;
  minNrowsNx = fv->size[0];
  for (i = 0; i < minNrowsNx; i++) {
    nt_re = fv_data[i].re;
  ...
  f_b_FFTImplementationCallback_r(b_fv, n2blue, costab, sintabinv, fv);
  fv_data = fv->data;
  f_emxFree_creal_T(&b_fv);
  i = wwc->size[0];
  for (k = nfft; k <= i; k++) {
    ...
    minNrowsNx = k - nfft;
    y_data[minNrowsNx].re = re_tmp * nt_im + nt_re * b_re_tmp;
    y_data[minNrowsNx].im = re_tmp * b_re_tmp - nt_re * nt_im;
  }
  f_emxFree_creal_T(&fv);
  f_emxFree_creal_T(&wwc);
}
                                     ...
                                     emxArray_real_T *sintabinv)
{
  ...
  f_emxInit_real_T(&costab1q);
  i = costab1q->size[0] * costab1q->size[1];
  costab1q->size[0] = 1;
  costab1q->size[1] = n + 1;
  f_emxEnsureCapacity_real_T(costab1q, i);
  ...
  if (!useRadix2) {
    ...
    costab->size[0] = 1;
    costab->size[1] = nd2 + 1;
    f_emxEnsureCapacity_real_T(costab, i);
    costab_data = costab->data;
    i = sintab->size[0] * sintab->size[1];
    sintab->size[0] = 1;
    sintab->size[1] = nd2 + 1;
    f_emxEnsureCapacity_real_T(sintab, i);
    sintab_data = sintab->data;
    costab_data[0] = 1.0;
    sintab_data[0] = 0.0;
    ...
    f_emxEnsureCapacity_real_T(sintabinv, i);
                                     ...
                                     emxArray_creal_T *y)
{
  ...
  iy = y->size[0];
  y->size[0] = n1_unsigned;
  f_emxEnsureCapacity_creal_T(y, iy);
  y_data = y->data;
  if (n1_unsigned > x->size[0]) {
    iy = y->size[0];
    y->size[0] = n1_unsigned;
...
/*
 * File trailer for f_FFTImplementationCallback.c
 *
 * [EOF]
 */
