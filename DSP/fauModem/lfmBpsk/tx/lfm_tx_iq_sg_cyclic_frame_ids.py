...
Revised from tx_iq_sg_cyclic_frame_ids.py: the packet-building step now
calls lfm_bpsk_tx.build_cyclic_tx_packet() (LFM sounding burst + guard +
RRC-shaped 1024-symbol training + 200-symbol data payload per physical
frame) instead of frame_ids_tx.build_tx_packet()'s old CP/pilot/block
frame. The hardware driver calls (bpsk_dma, dma_tx_sg_16m) are UNCHANGED
-- they only need a packed Q15 word buffer and a per-frame word count,
neither of which cares what the frame's internal structure is.

Dropped vs. the original: --plain-counter and --legacy-single-marker.
Both were specific to the old block-marker scheme (multiple markers
repeated across 4 data blocks); the new frame always carries exactly one
A5 5A id ~id marker at the start of its single data payload (see
lfm_bpsk_frame.data_bits), so there is nothing for those flags to select
between here.

NCO/carrier: bpsk_dma.set_nco_frequency() is what actually applies the
Fc=120kHz carrier in hardware -- the Python-side waveform this script
builds is real baseband (see lfm_bpsk_params.py's docstring), exactly
like the existing BPSK-only TX script. That confirms the baseband design
choice made when porting the RX side.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Cyclic SG MM2S TX packet, LFM+PRBS+BPSK frame-ID payloads")
    ...
    parser.add_argument("--packet-frames", type=int, default=36,
                        help="Number of physical LFM+BPSK frames in the cyclic DMA packet")
    ...
    print("=== EXTERNAL LOOPBACK IQ TX CONFIG (LFM+PRBS+BPSK) ===")
    ...
    print("  TX IQ mapping    : I=LFM+BPSK waveform, Q=0")
    print(f"  TX scale         : {args.tx_scale:g}"
          f"{' (peak-normalised, no clipping)' if args.tx_scale == 1.0 else ''}")
    ...
    print(f"  Payload mode     : A5 5A id ~id marker + PRNG data (see lfm_bpsk_frame.py)")
